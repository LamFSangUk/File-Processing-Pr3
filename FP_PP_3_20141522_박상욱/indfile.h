...
{public:
	...
	int Update (char * oldKey, RecType & record,int*delSize);
	int Create (char * name, int mode=ios::in|ios::out);
	void MakeIdxFile(char * fileName);
...
int TextIndexedFile<RecType>::Delete (RecType & record)
{
	...
	int ref = Index.Search(key);
	if (ref < 0) return -1;
	int delSize = DataFile . Delete(ref);
...
template <class RecType>
void TextIndexedFile<RecType>::MakeIdxFile(char * fileName){
	
	DelimFieldBuffer buffer ('|', STDMAXBUF);
	RecordFile <RecType> DatFile (buffer);

	int result;
	char * dataFileName, * indexFileName;
	result = SetFileName (fileName, dataFileName, indexFileName);

	TextIndex Index(10000);

	DatFile.Open (dataFileName, ios::in);
	while (1) {
		RecType temp;
		int recaddr = DatFile.Read (temp);
		if (recaddr == -1) break;

		Index.Insert(temp.Key(), recaddr);
	}
	DatFile.Close ();

	//Save into index file
	TextIndexBuffer idxbuf(15, 10000);
	BufferFile StdIdxFile (idxbuf);
	StdIdxFile.Create (indexFileName, ios::out | ios::trunc);
	StdIdxFile.Rewind ();
	
	idxbuf.Pack(Index);
	result = StdIdxFile.Write ();
	StdIdxFile.Close ();
}
