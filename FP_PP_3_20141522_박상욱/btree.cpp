...
{
	BTreeNode<char> *deleteNode, *parentNode;
	//Find the node to delete.
	deleteNode = FindLeaf(key); 
	
	bool Large_flag = false;
	keyType Large_key = deleteNode->LargestKey(), Large_key_merge;
	if (Large_key == key)
		Large_flag = true;
	
	//Delete the deletenode.	
	int result = deleteNode->Remove(key, recAddr); 
	if (result != -1) {       
		if (Large_flag) { 
			for (int i = 0; i < Height - 1; i++) {
				Nodes[i]->UpdateKey(Large_key, deleteNode->LargestKey());
				if (i > 0)
					Store(Nodes[i]);
			}
		}
		...
		for (int i = 0; i < Nodes[Height - 2]->NumKeys; i++) {
			...
			if (TN->RecAddr == deleteNode->RecAddr)
				continue;
			if (TN->Merge(deleteNode))
				break;
		}
		//Save the Parent Node.
		parentNode = Nodes[Height - 2];
		
		if (TN->LargestKey() == deleteNode->Keys[0]) {
			int temp1, temp2;
			for (int i = 0; i < parentNode->NumKeys; i++) {
				if (parentNode->Keys[i] == Large_key)       
					temp1 = i;
				if (parentNode->Keys[i] == Large_key_merge) 
					temp2 = i;
			}
			parentNode->RecAddrs[temp1] = parentNode->RecAddrs[temp2];
			parentNode->Remove(Large_key_merge, recAddr); 
		}
		else
			parentNode->Remove(Large_key, recAddr);
		Store(parentNode);
		Store(TN);	
	}
	return -1;
